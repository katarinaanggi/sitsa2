  ...
  <section class="section container-p-y container-xxl">
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">Daftar Pengeluaran Toko</h4>
        ...
        <div class="card-body">
          <a class="btn btn-primary mb-3" onclick="create()" style="color: #fff">+Tambah Pengeluaran</a><br /><br />
          <div id="read"></div>
          ...
          <table class="table table-inverse table-responsive table-hover" id="expenseTable" style="width:100%">
      ...
      <div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="titlemodal1">Tambah Expense</h5>
            <button
            type="button"
              class="btn-close"
      ...
      buttons: [
        {
          ...
          text: 'Excel',
          title: 'Rekap Pengeluaran Sri Ayu',
          footer: true, 
          exportOptions: {
            columns: [ 1, 2, 3, 4, 5 ],
            format: {
...
@section('script')
    <script>
      ...
      function create() {
        $.get(" {{ route('admin.add_expense') }} ", {}, function(data,status) {
          $("#titlemodal1").html('Tambah Expense');
          $("#addpage").html(data);
          $("#addModal").modal('show');
        });
      }
      ...
      function edit(id) {
        $.get("{{ route('admin.edit_expense', '')}}"+"/"+id, {}, function(data,status) {
          $("#exampleModalLabel1").html('Ubah Expense');
          $("#page").html(data);
      ...
      function update(id) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            $.ajax({
              type:"post",
              url:"{{ route('admin.update_expense', '')}}"+"/"+id,
              data: fd,
              contentType: false,
              processData: false,
              success: function(data){
      ...
      function destroy(id, pic) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            //url
            // let url = "{{ route('admin.delete_expense', ['id' => ':id']) }}";
            // url = url.replace(':id', id);
            //ajax
            $.ajax({
              type:"get",
              url:"{{ route('admin.delete_expense', '')}}"+"/"+id,
              success: function(data){
                //close modal and refresh table
                $('#expenseTable').DataTable().ajax.reload();
